...
async def get_github_readme(repo_url: str) -> str | None:
    ...
    headers = {"Accept": "application/vnd.github.v3.raw", "Authorization": f"token {settings.GITHUB_TOKEN}"}
...
async def get_readmes_for_repos(repo_urls: list[str]) -> list[tuple[str, str]]:
    ...
    readme_contents = await asyncio.gather(*[get_github_readme(url) for url in repo_urls])
