...
@pytest.mark.asyncio(loop_scope="function")
async def test_create_api_key_success(client: AsyncClient, authenticated_user: DBUser, db_session: AsyncSession):
    """Test POST /users/{user_id}/api-keys with valid data."""
    api_key_data = {"key_name": "Test API Key"}

    response = await client.post(f"/api/v1/users/{authenticated_user.id}/api-keys", json=api_key_data)
    ...
    assert "key_hash" not in data  # Should NOT be in response
    assert data["key_name"] == "Test API Key"
    assert data["user_id"] == str(authenticated_user.id)
    # Test attributes is present in response
    assert "attributes" in data
    assert "rate_limits" in data["attributes"]
    ...
    # Cleanup the API key that was created via the endpoint
    service = APIKeyService(db_session)
    await service.delete(api_key_id=uuid.UUID(data["id"]), permanent=True)


@pytest.mark.asyncio(loop_scope="function")
async def test_create_api_key_unauthenticated(client: AsyncClient):
    """Test POST /users/{user_id}/api-keys without authentication."""
    user_id = uuid.uuid4()
    api_key_data = {"key_name": "Test API Key"}
    ...
    response = await client.post(f"/api/v1/users/{user_id}/api-keys", json=api_key_data)
...
async def test_list_api_keys_success(client: AsyncClient, authenticated_user: DBUser, db_session: AsyncSession):
    ...
    assert len(data) == 2
    assert all(key["user_id"] == str(authenticated_user.id) for key in data)

    # Validate response structure - matches APIKeyOutput
    required_fields = {"id", "user_id", "key_name", "key_prefix", "created_at", "attributes"}
    forbidden_fields = {"key_hash", "api_key", "is_active", "updated_at"}  # These should never be in list responses
    ...
    for key in data:
        ...
        assert isinstance(key["attributes"]["rate_limits"], list)
        assert len(key["attributes"]["rate_limits"]) >= 1
...
async def test_list_api_keys_unauthenticated(client: AsyncClient):
    ...
    response = await client.get(f"/api/v1/users/{user_id}/api-keys")
...
async def test_delete_api_key_success(client: AsyncClient, authenticated_user: DBUser, db_session: AsyncSession):
    ...
    repository = APIKeyRepository(db_session)
    data = APIKeyInput(user_id=authenticated_user.id, key_name="Delete Test", is_active=True)
    created_key = await repository.create(data)
...
async def test_delete_api_key_nonexistent(client: AsyncClient, authenticated_user: DBUser):
    ...
    response = await client.delete(f"/api/v1/users/{authenticated_user.id}/api-keys/{fake_key_id}")
...
async def test_delete_api_key_unauthorized_different_user(
    ...
    repository = APIKeyRepository(db_session)
    data = APIKeyInput(user_id=other_user.id, key_name="Other User Key", is_active=True)
    ...
    assert response.status_code == 404  # Should be unauthorized/not found
    error_data = response.json()
    assert "permission" in error_data["detail"].lower() or "not found" in error_data["detail"].lower()

    # Cleanup the other user and key we created
...
async def test_delete_api_key_unauthenticated(client: AsyncClient):
    ...
    response = await client.delete(f"/api/v1/users/{user_id}/api-keys/{api_key_id}")
